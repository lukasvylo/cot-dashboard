...
# ── STAŽENÍ DAT PŘES CFTC SOCRATA API ───────────────────────────────────────
@st.cache_data(ttl=3600 * 6, show_spinner=False)
def fetch_cot_api():
    """Stáhne Supplemental COT data přes CFTC Public Reporting API (Socrata)."""
    ...
    try:
        ...
        df = df.dropna(subset=["report_date", "nc_long", "nc_short"])
...
<div class="dash-header">
  ...
  <div class="dash-sub">CFTC Public API · Supplemental Report · {datetime.now().strftime('%d.%m.%Y %H:%M')}</div>
</div>
""", unsafe_allow_html=True)

# ── NAČTENÍ DAT ──────────────────────────────────────────────────────────────
with st.spinner("Načítám Supplemental COT data z CFTC API…"):
    df_all = fetch_cot_api()

if df_all is None or df_all.empty:
    st.error("❌ Nepodařilo se načíst data z CFTC API. Zkus obnovit stránku za chvíli.")
    st.stop()
...
for c in COMMODITIES:
    ...
    summary.append({
        ...
        "delta":      float(sub["net"].iloc[-1]) - float(sub["net"].iloc[-2]) if len(sub) > 1 else 0,
...
if not summary:
    st.error("Žádná data nenalezena. Zkus obnovit stránku.")
    st.stop()
...
<div style='margin-top:1rem;font-family:IBM Plex Mono,monospace;font-size:10px;color:#3a4038;display:flex;gap:20px;flex-wrap:wrap'>
  <span><span style='color:#7ed957'>▲ 0–20</span> Bullish — spekulanti přeprodáni</span>
  <span><span style='color:#e05a3a'>▼ 80–100</span> Bearish — spekulanti překoupeni</span>
  <span><span style='color:#5a6358'>◆ 20–80</span> Neutrální</span>
</div>
""", unsafe_allow_html=True)
...
if "cot_detail" in st.session_state:
    ...
    if det:
        ...
        fig = make_subplots(
            ...
            subplot_titles=["Net pozice Non-Commercial", f"COT Index · {lb_label}"]
        )
...
<div style='margin-top:2.5rem;padding-top:1rem;border-top:1px solid #1a1e1b;
  ...
  <span>Zdroj: CFTC Supplemental CIT · publicreporting.cftc.gov/resource/4zgm-a668 · pátek ~15:30 EST</span>
